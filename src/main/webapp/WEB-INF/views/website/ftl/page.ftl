...
<#macro pager pager baseUrl parameterMap = {} maxShowPageCount = 4 showTxtNav = 1>
	...
	<#list parameterMap?keys as key>
		<#if parameterMap[key]?has_content>
	...
	<#if (pageCount > 1)>
		...
		<#if (endPageNumber < pageCount)>
			<#-- <a href="${baseUrl + "pageNo=" + (pageNumber + 2) + parameter}">...</a> -->
			<a>...</a>
...
<#-- 分页（Pager对象、链接URL、参数Map、最大页码显示数） -->
<#macro mopager pager baseUrl parameterMap = {} maxShowPageCount = 4 showTxtNav = 1>
	<#local pageNumber = pager.pageNo /> 
	<#local pageSize = pager.pageSize /> 
	<#local pageCount = pager.totalPage /> <#--总页数-->
	<#local parameter = "" />
	...
	<#if (pageCount > 1)>
		...
		<#-- 上一页 -->
		<#if (pageNumber > 1)>
			<a class="first" href="${prePageUrl}"><i></i> <#if showTxtNav=1></#if></a>
		<#else>
			<a class="first"><i></i><#if showTxtNav=1></#if></a>
		</#if>
			
			
		<#if (startPageNumber > 1)>
			<#-- <a href="${baseUrl + "pageNumber=" + 1 + parameter}">1</a> -->
			<#-- <a href="${baseUrl + "pageNumber=" + (pageNumber - 2) + parameter}">...</a> -->
			<#-- <a>...</a> -->
		</#if>
		...
		<#-- 下一页 -->
		<#if (pageNumber < pageCount)>
			<a class="last" href="${nextPageUrl}"><#if showTxtNav=1></#if><i></i></a>
		<#else>
			<a class="last"><#if showTxtNav=1></#if><i></i></a>
		</#if>
